...
class ContextContainer extends Container {
  ...
  getSelectedEnvName() {
    return this.state.selectedEnv >= 0
      ? this.getEnvs()[this.state.selectedEnv]
      : "";
  }
  ...
  getEnvironment(index) {
    ...
    return this.getVariables().reduce(
      (acc, curr) => ({ ...acc, ...getVar(curr) }),
      {}
    );
  }
  ...
  getSelectedEnvironment() {
    const envIndex =
      this.state.selectedEnv >= 0
        ? this.state.selectedEnv
        : this.getEnvs().length > 0
        ? 0
        : -1;
  ...
  setVariableNameAt(n, name) {
    ...
    const newVariables = R.map(
      R.ifElse(
        R.propEq("name", variable.name),
        R.assoc("name", name),
        R.identity
      ),
      this.getVariables()
  ...
  setVariableAt(env, i, props) {
    ...
    console.log(
      `setVariableAt() updating variable at index ${i}: ${JSON.stringify(
        existingVar
      )}`
    );
  ...
  setEnvironmentName(prevName, name) {
    const newVariables = this.getVariables().map(v => ({
      name: v.name,
      values: v.values.map((v, i) =>
        v.env == prevName ? { ...v, env: name } : v
      )
    }));
    this.setState({ vars: newVariables });
    return newVariables;
