...
class ItemList extends React.PureComponent {
  ...
  setName(index, value) {
    this.setState({ currentRef: null, input: null }, () => this.props.setName(index, value));
  }
  setValue(index, value) {
    this.setState({ currentRef: null, input: null }, () => this.props.setValue(index, value));
  }
  ...
  render() {
    ...
    return <div>
      {items.map((item, index) =>
        <RequestComp
          ref={index == items.length-1 ? { nameRef: this.nameRef, valueRef: this.valueRef } : null}
          component={item}
          {...this.props}
